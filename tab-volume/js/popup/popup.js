const toPromise = (fn) => new Promise((y, n) => fn((data, err) => err ? n(err) : y(data)));
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const [activeTab] = await toPromise((cb) => chrome.tabs.query({ active: true, currentWindow: true }, cb));
  const currentVolume = await emit({ name: 'get-tab-volume', tabId: activeTab.id });
